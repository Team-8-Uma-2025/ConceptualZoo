  ...
  router.post('/purchase', authenticateToken, async (req, res) => {
    try {
      ...
      if (!giftShopId || !products || !products.length || !cardNumber) {
        return res.status(400).json({ error: 'Gift shop, products, and card number are required' });
      }
  ...
  router.get('/transactions', authenticateToken, async (req, res) => {
    try {
      ...
      // If user is a manager, get all transactions or filter by gift shop
               ...
               (req.user.staffType === 'Gift Shop Clerk' || req.user.staffRole === 'Manager')) {
      ...
      rows.forEach(row => {
        if (!transactions[row.TransactionID]) {
          ...
          if (row.VisitorID && row.firstName) {
            ...
            transactions[row.TransactionID].visitorName = `${row.firstName} ${row.lastName}`;
          }
        }
