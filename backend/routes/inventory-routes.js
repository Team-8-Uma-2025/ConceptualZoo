...
module.exports = (pool) => {
  ...
  router.post('/', authenticateToken, async (req, res) => {
    try {
      ...
      if (!GiftShopID || !ProductID || ProductQuantityInStock === undefined) {
        return res.status(400).json({ error: 'GiftShopID, ProductID, and ProductQuantityInStock are required' });
      }
      ...
      if (existingCheck.length > 0) {
        return res.status(400).json({ error: 'This product is already in inventory for this gift shop' });
      }
  ...
  router.delete('/:id', authenticateToken, async (req, res) => {
    try {
      ...
      const [inventoryCheck] = await pool.query('SELECT * FROM inventory WHERE InventoryID = ?', [inventoryId]);
